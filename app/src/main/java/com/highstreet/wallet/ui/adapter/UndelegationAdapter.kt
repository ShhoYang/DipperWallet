...
class UndelegationAdapter : BasePagedAdapter<DelegationInfo>(R.layout.g_item_common) {
    ...
    override fun bindViewHolder(holder: ViewHolder, item: DelegationInfo, position: Int) {
        ...
        desc.append(getString(R.string.initAmount)).append("：").append(StringUtils.pdip2DIP(item.shares)).append("\n")
            .append(getString(R.string.balance)).append("：").append(StringUtils.pdip2DIP(item.balance)).append("\n")
            .append(getString(R.string.timeRemaining)).append("：").append(StringUtils.timeGap(item.completionTime))
